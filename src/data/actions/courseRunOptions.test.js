...
describe('courseRunOptions fetch courseRun actions', () => {
  ...
  it('handles fetch success', () => {
    mockClient.onOptions('http://localhost:18381/api/v1/course_runs/')
      .replyOnce(200, JSON.stringify(courseRunData));

    const expectedActions = [
      requestCourseRunOptions(),
      requestCourseRunOptionsSuccess(courseRunData),
    ];
    const store = mockStore();

    return store.dispatch(fetchCourseRunOptions()).then(() => {
